# -*- coding: utf-8 -*-
"""FlowForge runtime workflow models: status, condition, state, results.

Ported from ``leagent.workflow.base``. Only the runtime types the
executor operates on live here; workflow *structure* is defined in
:mod:`flowforge.engine.document`.
"""
...
class WorkflowState(BaseModel):
    ...
    def resolve_template(self, template: Any) -> Any:
        """Resolve ``${variable}`` template expressions in a value."""
        return _resolve_template(template, self._build_context())

    def evaluate_expression(self, expr: ConditionExpression) -> bool:
        """Evaluate a condition expression against current state."""
        return expr.evaluate(self._build_context())
...
def _resolve_field(expr: Any, context: dict[str, Any], default: Any = None) -> Any:
    ...
    if match:
        return _get_nested_value(context, match.group(1), default)
    if expr in context:
        return context[expr]
    parts = expr.split(".")
    if len(parts) > 1:
...
def _coerce_for_comparison(left: Any, right: Any) -> tuple[Any, Any]:
    """Coerce left/right to compatible types for comparison."""
...
def _resolve_template(template: Any, context: dict[str, Any]) -> Any:
    ...
    if isinstance(template, str):
        if TEMPLATE_PATTERN.fullmatch(template):
            path = TEMPLATE_PATTERN.fullmatch(template).group(1)  # type: ignore[union-attr]
...
# Forward-ref model rebuild for self-referencing ``conditions`` field.
