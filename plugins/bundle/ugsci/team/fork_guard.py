...
Adapted from OMP's ``shared/fork_guard.py``.
"""
...
def forks_integrated(
    ...
    - When no integration project can be resolved (non-git workspace, no
      pointer), treat as "no registry forks are possible" and allow.
...
must stay as written — do NOT rewrite it to dispatch). After merges
succeed, update state.json with the JSON boolean forks_integrated=true
(not a string). Then the gate resumes the original target phase —
do not re-dispatch expert workers.
"""
