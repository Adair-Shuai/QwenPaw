...
class EclipseReader(BaseReader):
    ...
    def extensions(self) -> tuple[str, ...]:
        return (".egrid", ".init", ".unrst", ".grdecl")
    ...
    def read(
        ...
        grd = xtgeo.grid_from_file(file_path, fformat="egrid")
        dims = grd.dimensions
        ...
        init_path = Path(file_path).with_suffix(".INIT")
        if init_path.exists():
        ...
        unrst_path = Path(file_path).with_suffix(".UNRST")
