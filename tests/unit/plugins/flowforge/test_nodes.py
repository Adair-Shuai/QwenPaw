# -*- coding: utf-8 -*-
"""Tests for flowforge.engine.nodes — registry + built-in node run() methods."""
...
from plugins.bundle.flowforge.engine.types import WorkflowState, WorkflowStatus
...
class TestNodeRegistry:
    def test_bootstrap_registers_all_builtins(self):
        reg = bootstrap()
        for ct in (
            "InputNode", "OutputNode", "ToolNode", "AgentNode",
            "ConditionNode", "LLMNode", "CodeNode",
        ):
            assert ct in reg
        assert len(reg) >= 7
...
class TestInputNode:
    def test_run_materialises_input(self):
        ...
        node = InputNode()
        out = _run(node.run("n1", {"inputs": {"name": "query"}}, {}, make_hidden(state)))
...
class TestToolNode:
    ...
    def test_run_callable_raises_returns_error(self):
        ...
        node = ToolNode()
        out = _run(node.run("n7", {"func": boom, "inputs": {}}, {}, make_hidden()))
        assert out.error is not None
...
class TestCodeNode:
    ...
    def test_run_with_input_value(self):
        ...
        out = _run(node.run(
            "n9",
            {"inputs": {"code": "result = sum(inputs.get('nums', []))", "nums": [1, 2, 3]}},
            {},
            make_hidden(),
        ))
    ...
    def test_run_invalid_code_returns_error(self):
        node = CodeNode()
        out = _run(node.run("n10", {"inputs": {"code": "1/0"}}, {}, make_hidden()))
        assert out.error is not None
...
class TestConditionNode:
    ...
    def test_invalid_expr_returns_error(self):
        ...
        out = _run(node.run(
            "n13",
            {"control": {"conditions": [{"then_node": "yes", "condition": {"left": "x"}}]}},
            {},
...
class TestAgentNode:
    def test_run_requires_runtime(self):
        node = AgentNode()
        out = _run(node.run("n14", {"inputs": {"agent_id": "a1", "query": "hi"}}, {}, make_hidden()))
        assert out.error is not None
...
class TestLLMNode:
    def test_run_requires_llm_service(self):
        node = LLMNode()
        out = _run(node.run("n16", {"inputs": {"prompt": "hi"}}, {}, make_hidden()))
        assert out.error is not None
...
class TestNodeRunner:
    ...
    def test_unknown_class_type_raises(self):
        ...
        with pytest.raises(Exception):
            _run(runner.run("n1", {"class_type": "GhostNode", "inputs": {}}, {}, make_hidden()))
    ...
    def test_node_failure_emits_failed_status(self):
        ...
        progress = ProgressRegistry(prompt_id="test")
        runner = NodeRunner(registry=reg, progress=progress)
        def boom(**kw): raise ValueError("fail")
        # ToolNode catches exceptions internally and returns NodeOutput(error=...);
        # NodeRunner surfaces that as a FAILED progress status (no re-raise).
        result = _run(runner.run("n1", {"class_type": "ToolNode", "func": boom, "inputs": {}}, {}, make_hidden()))
