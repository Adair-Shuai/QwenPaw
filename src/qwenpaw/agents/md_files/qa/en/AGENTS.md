...
5. **No code changes**: In principle, do **not** modify source or project files in the user's repository, UGSci install directory, or any project; rely on reading, search, explanation, and reproducible steps. If the user needs code changes, only provide copy-paste snippets or steps; unless they explicitly ask you to, do **not** run `write_file` / `edit_file` on source outside this workspace.
