---
name: officecli-reference
description: "OfficeCLI reference guide: L1(read)→L2(DOM edit)→L3(raw XML) strategy, command syntax, element types, property formats, and common pitfalls for .docx/.xlsx/.pptx. Load this skill when working with Office documents that need advanced operations beyond the basic office_* tools."
metadata:
...
> **QwenPaw Tool Mapping:** This skill provides CLI reference for OfficeCLI. QwenPaw wraps the most common operations as `office_*` tool functions. Use the table below to map CLI commands to QwenPaw tools.
>
> | CLI Command | QwenPaw Tool (default enabled) | QwenPaw Tool (opt-in) |
> |-------------|-------------------------------|----------------------|
> | `create` | `office_create_document` | |
> | `add` | `office_add_element` | |
> | `set` | `office_set_properties` | |
> | `get` | `office_get_element` | |
> | `query` | `office_query_elements` | |
> | `remove` | `office_remove_element` | |
> | `move` | | `office_move_element` |
> | `swap` | | `office_swap_elements` |
> | `view text` | | `office_get_text` |
> | `view stats` | | `office_get_stats` |
> | `view html/outline/issues` | `office_view_document` | |
> | `view screenshot` | `office_view_screenshot` | |
> | `import` | | `office_import_data` |
> | `refresh` | | `office_refresh_fields` |
> | `validate` | `office_validate_document` | |
> | `merge` | `office_merge_template` | |
> | `batch` | `office_batch_operations` | |
> | `raw` | | `office_raw_get` |
> | `raw-set` | | `office_raw_set` |
>
> For operations not covered by a QwenPaw tool (e.g. `open`/`close`/`watch`/`dump`), use `execute_shell_command` to call officecli directly.
