...
def queue_component_update(component: str) -> dict[str, Any]:
    ...
    try:
        ...
        if component not in plans:
            return {"component": component, "queued": False, "reason": "up-to-date"}
        with _PENDING_LOCK:
            ...
            if _PENDING_UPDATES_PATH.is_file():
                try:
                    existing = json.loads(_PENDING_UPDATES_PATH.read_text(encoding="utf-8"))
                    if isinstance(existing, dict) and isinstance(existing.get("components"), list):
                        pending = existing
                except (OSError, json.JSONDecodeError):
                    pass
            pending["components"] = sorted({str(item) for item in pending["components"]} | {component})
            temporary = _PENDING_UPDATES_PATH.with_name(f".{_PENDING_UPDATES_PATH.name}.{os.getpid()}.{threading.get_ident()}.tmp")
            temporary.write_text(json.dumps(pending, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")
            os.replace(temporary, _PENDING_UPDATES_PATH)
        return {"component": component, "queued": True, "restart_required": True}
    finally:
...
class ComponentUpdateService:
    ...
    def _install_component(self, component: str) -> dict[str, Any]:
        ...
        if not isinstance(entry, dict):
            raise ComponentUpdateError(f"component is missing from manifest: {component}")
        self.updater.recover_interrupted_activation(
            component,
            destination,
            expected_files=entry.get("files") if isinstance(entry.get("files"), dict) else None,
...
def run_startup_updates() -> dict[str, Any]:
    ...
    if _PENDING_UPDATES_PATH.is_file():
        try:
            payload = json.loads(_PENDING_UPDATES_PATH.read_text(encoding="utf-8"))
            if isinstance(payload, dict) and isinstance(payload.get("components"), list):
                pending_components = [str(item) for item in payload["components"]]
        except (OSError, json.JSONDecodeError):
            logger.warning("Pending component update queue is invalid", exc_info=True)
    ...
    try:
        ...
        selected = available if mode == "startup" else available & set(pending_components)
        ...
        if pending_components:
            if remaining_pending:
                temporary = _PENDING_UPDATES_PATH.with_name(f".{_PENDING_UPDATES_PATH.name}.{os.getpid()}.tmp")
                temporary.write_text(json.dumps({"schema_version": 1, "components": sorted(remaining_pending)}, sort_keys=True) + "\n", encoding="utf-8")
                os.replace(temporary, _PENDING_UPDATES_PATH)
            else:
