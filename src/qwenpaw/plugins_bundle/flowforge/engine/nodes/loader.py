...
1. **Built-ins** — :mod:`.builtin` (curated list).
2. **Entrypoints** — installed Python distributions exposing an entrypoint
   in the ``flowforge.workflow.nodes`` group. The target must be either a
...
async def load_entrypoints(registry: NodeRegistry | None = None) -> list[str]:
    """Load any installed ``flowforge.workflow.nodes`` entry points."""
    reg = registry or get_registry()
    registered: list[str] = []
    try:
...
async def _load_module(path: Path, registry: NodeRegistry) -> list[str]:
    ...
    if path.is_dir():
        ...
        if not init_py.exists():
            return []
        spec = importlib.util.spec_from_file_location(f"flowforge_custom.{path.name}", init_py)
    elif path.suffix == ".py":
        spec = importlib.util.spec_from_file_location(f"flowforge_custom.{path.stem}", path)
    else:
        return []
...
def _run_file(path: Path) -> None:
    spec = importlib.util.spec_from_file_location(f"flowforge_prestart.{path.stem}", path)
    if spec is None or spec.loader is None:
        return
    module = importlib.util.module_from_spec(spec)
    try:
...
async def bootstrap(
    *,
    ...
    :func:`.tool_factory.register_tool_nodes`.
    ...
    :func:`.agent_node_factory.register_agent_nodes`.
    """
    reg = registry or get_registry()
