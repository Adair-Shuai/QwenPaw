...
// @ts-expect-error — @xyflow/react is bundled by vite, no types in host
import {
  ReactFlow,
  Background,
  Controls,
...
function NodeCard({ id, data, selected }: NodeProps) {
  ...
  const Tag = antd?.Tag;
  const classType: string = data?.class_type || data?.type || "ToolNode";
  const label: string = data?.label || classType;
  const color = NODE_COLORS[classType] || "#1677ff";
  const status: string = data?._status || "pending";
  const statusColor =
    status === "completed"
      ? "#52c41a"
      : status === "running"
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement("span", { style: { fontSize: 16 } }, data?.icon || "🔧"),
      React.createElement(
        "strong",
        { style: { color } },
        label,
...
function FlowListPage({ onEdit, onRun }: { onEdit: (id: string) => void; onRun: (id: string) => void }) {
  const antd = getHost().antd;
  const { Table, Button, Space, Input, Modal, message } = antd;
  ...
  return React.createElement(
    "div",
    { style: { padding: 24 } },
    React.createElement(antd.PageHeader, {
      title: "工作流",
      subtitle: "可视化 DAG 工作流引擎",
