...
def _chunk(payload: dict[str, Any], *, error: bool = False) -> Any:
    ...
    return ToolChunk(is_last=True, state=ToolResultState.ERROR if error else ToolResultState.SUCCESS, content=[TextBlock(type="text", text=json.dumps(payload, ensure_ascii=False, indent=2))])
...
def _run(capability_id: str, request: Any, method: str, assumptions: list[str]) -> Any:
    try:
        ...
        return _chunk(_service.execute(capability_id, adapter, request, method=method, assumptions=assumptions).to_dict())
...
async def ugsci_bayesian_normal_estimate(
    observations: list[float], prior_mean: float = 0.0, prior_std: float = 10.0,
    observation_std: float = 1.0, draws: int = 1000, tune: int = 500, seed: int = 42,
) -> Any:
    ...
    try:
        ...
        request = BayesianNormalRequest([float(value) for value in observations], float(prior_mean), float(prior_std), float(observation_std), draws, tune, seed)
    except DomainError as exc:
        return _chunk(exc.to_dict(), error=True)
    except (TypeError, ValueError) as exc:
        return _chunk(DomainError(DomainErrorCode.INVALID_INPUT, f"Invalid stochastic input: {exc}").to_dict(), error=True)
    return _run("statistics.bayesian.normal_mean", request, "mcmc_nuts", ["Normal likelihood with known observation standard deviation", "Seeded sampling is reproducible but remains stochastic"])


async def ugsci_multiobjective_quadratic(
    centers: list[list[float]], bounds: list[list[float]], population_size: int = 80,
    generations: int = 100, seed: int = 42,
) -> Any:
    ...
    try:
        ...
        request = MultiObjectiveQuadraticRequest([[float(value) for value in row] for row in centers], [[float(value) for value in row] for row in bounds], population_size, generations, seed)
    except DomainError as exc:
        return _chunk(exc.to_dict(), error=True)
    except (TypeError, ValueError) as exc:
        return _chunk(DomainError(DomainErrorCode.INVALID_INPUT, f"Invalid stochastic input: {exc}").to_dict(), error=True)
    return _run("optimization.quadratic.pareto", request, "nsga2", ["Seeded NSGA-II is reproducible for a fixed Provider version but remains stochastic"])
