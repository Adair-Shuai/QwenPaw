...
def _write_research_mode_config(
    ...
    try:
        ...
        try:
            from qwenpaw.config.utils import _agent_config_cache, _agent_config_lock
...
def _register_research_mode(api) -> None:
    ...
    from qwenpaw.modes.base import AgentMode, ModeGatedHook
...
class UGSciResearchPlugin:
    ...
    def register(self, api) -> None:
        ...
        # 3. Sync skills to the shared skill pool (not individual workspaces).
        #    This makes research skills available to all agents via the pool
        #    without auto-injecting them into every agent.
        try:
            api.register_startup_hook(
                hook_name="ugsci_research_sync_skills_to_pool",
        ...
        try:
            api.register_uninstall_hook(
                hook_name="ugsci_research_remove_pool_skills",
    ...
    async def _handle_research_command(
        ...
        if action == "on":
            ...
            return Msg(
                name="assistant",
                role="assistant",
                content=[TextBlock(type="text", text=status_text)],
            )
        else:  # toggle
