...
- **默认永久保留**：`scroll_config.history_retention_days` 默认为 `0`，不会自动删除历史。只有显式设置为正数时，导入才会跳过早于该窗口的消息，并在启动/退出时清理对应的 `history.db` 行。把有限窗口改回 `0` 后，先前因保留期跳过但仍存在于 session JSON 的消息会重新导入。
...
| `scroll_config.history_retention_days`           | `0`            | 默认永久保留；只有显式设置为正数时才自动清理早于该天数的历史行。                   |
