...
import { CodeOutlined, EyeOutlined, DownloadOutlined, ReloadOutlined } from "@ant-design/icons";
import { useTranslation } from "react-i18next";
import type { RendererContext } from "../types";

type ViewMode = "preview" | "code";

const SandpackRenderer: React.FC<RendererContext> = ({ artifact, theme, workspace }) => {
  ...
  if (viewMode === "code") {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        <Toolbar viewMode={viewMode} setViewMode={setViewMode} artifact={artifact} workspace={workspace} t={t} />
        <pre style={{ margin: 0, padding: 12, fontSize: 13, flex: 1, overflow: "auto", whiteSpace: "pre-wrap" }}>
          {code}
        </pre>
      </div>
    );
...
const Toolbar: React.FC<any> = ({ viewMode, setViewMode, artifact, workspace, t }) => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 8px", borderBottom: "1px solid #f0f0f0", flexShrink: 0 }}>
    <Segmented size="small" value={viewMode} onChange={(v) => setViewMode(v as ViewMode)} options={[
