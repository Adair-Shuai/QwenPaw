...
const FallbackRenderer: React.FC<RendererContext> = ({ artifact, theme, workspace }) => {
  ...
  return (
    <div style={{
      ...
      <Typography.Text type="secondary">
        {t("workspace.unsupportedType", { type: artifact.mimeType || artifact.extension || "unknown" })}
      </Typography.Text>
      <Typography.Text style={{ fontSize: 12, color: "#999" }}>
        {artifact.title}
      </Typography.Text>
      {artifact.binaryUrl && (
        <Button type="primary" icon={<DownloadOutlined />} onClick={() => workspace.download?.(artifact)}>
