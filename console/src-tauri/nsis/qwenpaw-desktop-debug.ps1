...
if ($desktop) {
  Write-Host "Existing UGSci Desktop process detected; quit it from the tray before reproducing startup issues."
  $desktop = $null
} else {
  ...
  while ($true) {
    ...
    if ($desktop) {
      ...
      if ($desktop.HasExited) {
        Write-Host "UGSci Desktop exited with code $($desktop.ExitCode)."
        Show-DesktopOutput
