...
$LauncherBat = Join-Path $EnvRoot "UGSci Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
$DebugBat = Join-Path $EnvRoot "UGSci Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
echo [Launch] Starting UGSci Desktop with log-level=%QWENPAW_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m qwenpaw desktop --log-level %QWENPAW_LOG_LEVEL%
echo.
echo [Exit] UGSci Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "UGSci Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\UGSci Desktop.bat"
...
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "UGSci-Setup-$Version.exe"
