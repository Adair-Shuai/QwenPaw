#!/usr/bin/env bash
# One-click build: console -> conda-pack -> UGSci.app. Run from repo root.
...
<dict>
  ...
  <key>NSDesktopFolderUsageDescription</key><string>UGSci may access files in your Desktop folder if you use file-related features. You can choose Don'\''t Allow; the app will still run with limited file access.</string>
</dict>
</plist>
INFOPLIST
...
if [[ -n "${CREATE_ZIP}" ]]; then
  ZIP_NAME="${DIST}/UGSci-${VERSION}-macOS.zip"
