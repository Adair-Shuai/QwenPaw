...
# 产出: dist/UGSci-Setup-<version>.exe
# 创建两个启动器：
#   - UGSci Desktop.vbs (静默启动，无终端窗口)
#   - UGSci Desktop (Debug).bat (显示终端，便于调试)
```
...
用户从 Release 等渠道下载的 UGSci macOS 应用（zip 内的 .app）未经过 Apple 公证，可能看到「Apple 无法验证“UGSci”是否包含可能危害 Mac 安全或泄漏隐私的恶意软件」。可按以下方式打开：

- **右键打开（推荐）**
  在 UGSci 应用上 **右键（或 Control + 点击）** → 选 **「打开」** → 在弹窗里再点一次 **「打开」**。即表示你确认运行该应用，Gatekeeper 会放行，之后双击即可正常打开。

- **在系统设置里放行**
  若仍被拦截，进入 **系统设置 → 隐私与安全性**，往下找到类似「已阻止使用 UGSci，因为无法验证开发者」的提示，点 **「仍要打开」** 或 **「允许」** 即可。

- **用终端去掉隔离属性（不推荐普通用户）**
  在终端执行：`xattr -cr /Applications/UGSci.app`（或解压后 .app 的实际路径）。会去掉「从互联网下载」的隔离标记，一般就不再弹恶意软件提示，但不如「右键 → 打开」安全、可控。
...
| `build_macos.sh` | 一键：构建 wheel → build_common → 解压到 UGSci.app；可选打 zip |
