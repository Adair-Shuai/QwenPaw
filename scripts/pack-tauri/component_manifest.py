...
from component_common import file_inventory, safe_relative_path, read_plugin_metadata
...
def validate_manifest(
    ...
    *,
    ...
    for field in ("product", "channel", "target", "core_min_version", "components"):
        if not manifest.get(field):
            raise ValueError(f"manifest missing {field}")
    target = str(manifest["target"])
    if expected_target is not None and target != expected_target:
        raise ValueError(f"manifest target {target!r} does not match {expected_target!r}")
    if core_version is not None and _version(core_version, "core_version") < _version(
        manifest["core_min_version"], "core_min_version",
    ):
    ...
    if component_root is not None and len(components) != 1:
        raise ValueError("component_root validation requires exactly one component")

    for component_id, component in components.items():
        if not isinstance(component_id, str) or not component_id.strip():
            raise ValueError("component id must be a non-empty string")
        if component_id in {".", ".."} or any(char in component_id for char in ("/", "\\", "\x00")):
        ...
        if not isinstance(files, dict):
            raise ValueError(f"component {component_id!r} files must be an object")
        seen: set[str] = set()
        for relative, metadata in files.items():
            normalized = safe_relative_path(str(relative))
            if normalized != relative or normalized in seen:
                raise ValueError(f"invalid or duplicate component path: {relative!r}")
            seen.add(normalized)
            if (
                ...
                raise ValueError(f"invalid metadata for {component_id}/{relative}")
            digest = metadata.get("sha256")
            if not isinstance(digest, str) or not _SHA256.fullmatch(digest):
                raise ValueError(f"invalid sha256 for {component_id}/{relative}")
        ...
        if full is not None:
            ...
            if type(full.get("size")) is not int or full["size"] < 0:
                raise ValueError(f"invalid full artifact size for {component_id}")
            if not isinstance(full.get("sha256"), str) or not _SHA256.fullmatch(full["sha256"]):
                raise ValueError(f"invalid full artifact sha256 for {component_id}")
            if not isinstance(full.get("signature"), str) or not full["signature"].strip():
                raise ValueError(f"invalid full artifact signature for {component_id}")
        deltas = component.get("deltas", [])
        if not isinstance(deltas, list):
            raise ValueError(f"invalid deltas for {component_id}")
        for delta in deltas:
            if not isinstance(delta, dict) or not all(delta.get(key) for key in ("from", "url", "sha256", "signature")):
                raise ValueError(f"invalid delta metadata for {component_id}")
            _version(delta["from"], f"{component_id}.delta.from")
            if type(delta.get("size")) is not int or delta["size"] < 0 or not _SHA256.fullmatch(str(delta["sha256"])):
        ...
        if min_core is not None:
            ...
            if core_version is not None and _version(core_version, "core_version") < _version(min_core, f"{component_id}.min_core_version"):
                raise ValueError(f"core version is below {component_id} minimum")
